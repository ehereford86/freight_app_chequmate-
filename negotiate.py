...
def _fuel_costs_loaded_miles(loaded_miles: float, origin_state: str | None, fuel_mode: str | None) -> tuple[float, float, dict]:
    ...
    if diesel_price is None:
        # Keep structure stable for UI (never return null fuel)
        fuel_per_mile = 0.0
        fuel_total = 0.0
        meta_out = {
            "ok": False,
...
async def broker_negotiate(load_id: int, request: Request, u=Depends(require_broker_approved())):
    """
    Fair-market negotiation:
    - Uses FairRatePolicy assumptions (cost-per-mile + margins + caps)
    - Uses EIA diesel weekly (national by default, optional origin_state mapping)
    - Returns a COMPLETE summary so the UI doesn't need to scrape Raw JSON
    """
    ...
    # Optional fuel regional selection (UI can pass these)
    ...
    # Driver CPM is the only "variable" a broker might push; everything else is policy-based.
    ...
    # Guardrails: driver CPM threshold requires override reason (policy-based fairness)
    if driver_cpm > policy.driver_reason_threshold and not override_reason:
        warnings.append("Driver CPM exceeds threshold")

    # -----------------------------
    # Cost model (fair-market assumptions)
    # -----------------------------
    ...
    fuel_per_mile, fuel_total, fuel_obj = _fuel_costs_loaded_miles(loaded_miles, origin_state=origin_state, fuel_mode=fuel_mode)

    # Carrier cost basis (what it truly costs to run the trip)
    carrier_cost_subtotal = driver_total + carrier_operating_cost + carrier_accessorials + fuel_total

    # Carrier needs margin (policy default) to stay solvent and "market-real"
    carrier_revenue = carrier_cost_subtotal * (1.0 + policy.default_carrier_margin_pct)

    # Dispatcher cut on carrier revenue
    dispatch_fee = carrier_revenue * policy.default_dispatch_pct

    # Broker cost is carrier revenue + dispatch fee
    broker_cost = carrier_revenue + dispatch_fee

    # Customer rate (broker offer) adds broker margin (policy default)
    ...
    # Presentational “linehaul” (what customer pays excluding fuel surcharge)
    ...
    # What the UI should display without digging through JSON
    selected = {
        # Inputs the broker may influence
        ...
        # CORE SUMMARY FIELDS (for the Broker UI card)
    ...
    # Apply = write ONLY numeric fields (keep ratecon_terms clean)
    if apply_to_load:
